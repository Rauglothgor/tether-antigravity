...
- [2026-01-13] - Error: Repeatedly deleted `DECISIONS.md` during root cleanup by missing it in the `move-item` command. - Correction: Standardized the "Reference Grade" structure to include clean templates in the root `.planning/` while archiving internal history in `.gsd/meta/`. NEVER run `rmdir -Force` on `.planning/` in this repository without absolute confirmation of template safety.
